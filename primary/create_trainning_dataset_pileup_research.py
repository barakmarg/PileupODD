...
def preprocess_for_model(particles: pl.DataFrame, tracks: pl.DataFrame, calo_hits: pl.DataFrame,
    ...
    """
    Aggregates the number of cells per cluster (all vertices).
    """
    import psutil
    import os
    ...
    # Capture pid -> vertex_primary for ALL particles (incl. pileup). Used later
    # by cluster_vertex_primary_deps regardless of vertex filtering.
...
def run_preprocessing_pipeline_all_vertices_chunked(
    ...
    and write outputs in small event chunks to limit memory.

    Each chunk is written as {key}-{file_idx:05d}-chunk{chunk_idx:03d}.parquet
    ...
    for i in tqdm.tqdm(number_of_files):
        ...
        total_chunks = (len(event_ids) + chunk_size - 1) // chunk_size
        if total_chunks == 1:
            chunk_ids = event_ids
